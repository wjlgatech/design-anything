...
| **Garment** | ✅ v0.2 (`pattern_gate.py` + `dxf_aama.py`) | F1 pieces + grain · F2 fabric fit (bbox-level) · F3 seam allowance · F4 zero-waste marker efficiency · F5 fit tables vs [`data/garment.yml`](../data/garment.yml) · F6 seam/symmetry pairs. S/M/L grade run gated per size; DXF-AAMA export round-trip verified. Not a muslin. Roadmap: GarmentCode emission (needs upstream lib to verify honestly) |
