...
| **M7 — Game/sim gate** | glTF validity, poly budget, true-scale units, collision node; golden arena passes; 7 known-bad mutations fail (`tests/test_scene_gate.py`). | ✅ v0.8.0 |
| **M8 — IFC deepening** | `pipeline/ifc_export.py` emits IFC4 SPF (semantic: spatial structure, space areas, door widths; deterministic GlobalIds); golden studio flat round-trips always, ifcopenshell deep-checks when installed (skip-not-fail, absence reported). Geometry solids roadmap. | ✅ v0.10.0 |
| **M9 — Knowledge map** | `scripts/build_graph.py` compiles data/*.yml → `docs/graph.json` (drift-gated in `make check`); interactive force-map at `docs/map.html`, deployed via GitHub Pages on every merge. | ✅ v0.10.0 |
...
| **M11 — Body-fit expansion** | Eyewear ships the pattern: `data/bodyfit.yml` tables + `pipeline/bodyfit_gate.py` (completeness, ranges, optical alignment, print floor) + golden fit-spec AND printable temple, each passing its gate. Dental/prosthetics/footwear reuse the pattern (M11b+). | ✅ v0.10.0 |
| **M12 — Pattern-gate deepening** | DXF-AAMA export (round-trip verified), S/M/L grade run gated per size (ASTM D5585 lineage), seam/symmetry-pair matching (F6) — `tests/test_dxf_and_grading.py`. GarmentCode emission deferred to M12b: without the upstream lib, an emitter is an unverifiable claim. | ✅ v0.9.0 |
...
| **M17 — Satellites** | Every `repo:`-marked gold cited repo gets a SHA-pinned deep digest + a generated skill, compiled by `scripts/satellites.py` (sync=network, build=offline drift gate in `make check`); staleness computed, never claimed (`tests/test_satellites.py`); weekly freshness workflow opens human-gated PRs; satellites join the knowledge map. Design: [docs/SATELLITES.md](docs/SATELLITES.md). | ✅ v0.11.0 |
| **M19 — Dogfood + M12b** | First live end-to-end run of /design-anything (novel hex brief → compose → READY first try), banked as golden #6 per the reflect protocol; GarmentCode emitter ships with round-trip verification + skip-not-fail pygarment deep-check (measured: the PyPI wheel lacks `pygarment.pattern` — repo checkout required); freshness workflow live-verified. | ✅ v0.13.0 |
| **M18 — Flagship ⇄ satellites** | `/design-anything` routes to every satellite via a GENERATED §2a table (compiled from `data/satellites.yml`, drift-gated); backbone carries only the durable rules (matched-only loading, STALE ⇒ re-verify, untrusted-regardless-of-stars, our-gates-judge); reachability CI-asserted; satellite-routing eval scenario added. | ✅ v0.12.0 |
| **M16 — OOP + AI-native refactor** | Gates share the `Gate` seam (`pipeline/gate.py`: CheckResult, named checks, one report/CLI/exit surface); god-functions eliminated; typing 100%; `anyagent analyze` 50→83; AI-native self-audit (`data/ainative.yml` + `scripts/ainative.py`, 12 principles with in-repo evidence) gated inside `make check`. | ✅ v0.7.0 |
